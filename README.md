...
## 💊 Интеграция с «Честным знаком» / MDLP Sandbox


В Medical Diary добавлена серверная интеграция с тестовым контуром MDLP. При сканировании Data Matrix приложение разбирает GTIN/серийный номер и может выполнить запрос 8.3.3 — поиск по общедоступному реестру КИЗ по списку SGTIN.

Для включения sandbox добавьте в локальный `.env`:

```env
MDLP_ENABLED=true
MDLP_MODE=sandbox
MDLP_BASE_URL=https://api.sb.mdlp.crpt.ru
MDLP_TOKEN=<динамический токен sandbox>
MDLP_TIMEOUT=15
MDLP_MAX_RESPONSE_BYTES=1048576
```

`MDLP_TOKEN` — секрет и не должен попадать в Git, браузер или audit log. Для резидента РФ MDLP использует динамическую авторизацию; получение session token выполняется штатными средствами MDLP с ГОСТ-подписью. Клиент Medical Diary принимает уже полученный динамический token и использует его только на сервере.

Песочница доступна зарегистрированным участникам по адресу `https://sb.mdlp.crpt.ru/`; доступ к API незарегистрированному пользователю не предоставляется. Для API используется `https://api.sb.mdlp.crpt.ru`. После настройки токена запросы выполняются сервером приложения, а не Safari.

> **Важно:** без `MDLP_TOKEN` распознавание Data Matrix всё равно работает, но запрос в MDLP будет отмечен как недоступный. Ручное добавление лекарства не блокируется.
...
Справочная БД лекарств работает отдельно от пользовательской SQLite БД дневника и использует PostgreSQL 15+. Связка выполняется так:

`GTIN → номер РУ → карточка ГРЛС`.

В справочнике хранятся торговое наименование, МНН, лекарственная форма, дозировка, производитель, держатель РУ и статус. Таблица GTIN допускает несколько GTIN на один препарат.

Источники:
- ГРЛС Минздрава — ZIP/XLSX с реестром лекарственных средств;
- открытые данные МДЛП/Датамаркета — CSV со связкой GTIN и номера РУ.

Актуальный набор МДЛП публикуется Датамаркетом через endpoint `.../data/latest`; загрузчик использует этот официальный endpoint по умолчанию. Конкретный URL можно переопределить через `MDLP_EXPORT_URL`.
Для ГРЛС задаётся актуальный URL выгрузки через `GRLS_EXPORT_URL`.
...
```env
DRUG_DB_PASSWORD=сложный_пароль_postgres
GRLS_EXPORT_URL=<прямая ссылка на актуальный ZIP ГРЛС>
# MDLP_EXPORT_URL не обязателен: по умолчанию используется официальный /data/latest
```

`GRLS_EXPORT_URL` должен указывать непосредственно на выгрузку ZIP. Если он не задан, импорт ГРЛС пропускается. Загрузчик запускается ежедневно в 03:00 по `Europe/Moscow`; на каждом цикле проверяются ГРЛС и МДЛП.
...
PostgreSQL не публикуется наружу. Приложение обращается к нему только по внутренней Docker-сети. Загрузчик выполняется отдельным контейнером и пишет историю запусков в `drug_import_runs`.
...
Ответ при наличии записи содержит `trade_name`, `inn`, `dosage_form`, `dosage_value`, `manufacturer`, `holder`, `reg_number`, `package_desc` и `gtin`.

### Важное ограничение

Дата покупки не является справочным свойством ГРЛС/МДЛП — её вводит пользователь. Срок годности и количество единиц в конкретной упаковке относятся к упаковке/маркировке и не должны подменяться данными карточки препарата. Их следует хранить в пользовательской таблице конкретной упаковки, когда соответствующий источник действительно предоставляет эти значения.
