...
Портал Датамаркета публикует версии CSV отдельно; поэтому URL конкретной выгрузки задаётся через `MDLP_EXPORT_URL`, а не зашивается в код. Для ГРЛС аналогично задаётся актуальный URL выгрузки.
...
GRLS_EXPORT_URL=
MDLP_EXPORT_URL=
DRUGDB_UPDATE_INTERVAL_SECONDS=86400
```

`GRLS_EXPORT_URL` и `MDLP_EXPORT_URL` должны указывать непосредственно на файлы выгрузок. Если URL не задан, соответствующий импорт пропускается.
