...
Важно: в CSV поле `prod_name` — **МНН**, а поле `inn` — **ИНН организации**.
В карточке приложения историческое поле `inn` продолжает означать МНН для
совместимости интерфейса; ИНН организации туда не подставляется.
...
(по умолчанию 604800 секунд) из официального endpoint Датамаркета.
...
```env
MDLP_EXPORT_URL=<прямая ссылка на актуальную CSV-выгрузку>
